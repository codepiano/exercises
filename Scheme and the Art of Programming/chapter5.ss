...
(define (poly-quotient p1 p2)
    (cond ((zero-poly? p1) the-zero-poly)
          ...
          (else (let ((dg1 (degree p1))
                      (dg2 (degree p2)))
                      (cond ((> dg2 dg1) (error "degree of divine " p1 " is less than divisor" p2))
                            (else (let ((d1 (/ (leading-coef p1) (leading-coef p2)))
                                        (d2 (- dg1 dg2)))
                                  (p* p2 (poly-cons d2 d1 the-zero-poly)))))))))

(define p3 (poly-cons 3 5 (poly-cons 2 -3 (poly-cons 1 1 (poly-cons 0 -17 the-zero-poly)))))
(define p4 (poly-cons 3 5 (poly-cons 2 -3 (poly-cons 1 1 (poly-cons 0 -17 the-zero-poly)))))
